...
def resonator_spectroscopy(platform, qubit, settings, folder):
    ...
    freqrange = variable_resolution_scanrange(settings["lowres_width"], settings["lowres_step"], settings["highres_width"], settings["highres_step"])
    freqrange = freqrange + platform.settings["characterization"]["single_qubit"][qubit]["resonator_freq"]
    powrange = np.flip(np.arange(settings["min_power"], settings["max_power"],settings["step_power"]))
    platform.start()
    voltages = []
    freqs = []
    powers = []
    for freq in freqrange:
        for power in powrange:
            platform.qrm[qubit].set_device_parameter("out0_in0_lo_freq", freq + ro_pulse.frequency)
