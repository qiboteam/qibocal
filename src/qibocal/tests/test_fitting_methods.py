...
from qibocal.fitting.methods import lorentzian_fit,t1_fit,flipping_fit,drag_tunning_fit
from qibocal.fitting.utils import lorenzian, ramsey,flipping,cos
from qibocal.fitting.methods import rabi_fit,ramsey_fit
from qibocal.fitting.utils import rabi,exp
import logging
...
def test_lorentzian_fit(name, label, nqubits, amplitude_sign,caplog):
    ...
    np.testing.assert_allclose(abs(fit.get_values("popt0")[0]), abs(amplitude), rtol=0.1)
...
def test_rabi_fit( label, nqubits, amplitude_sign,caplog):
    ...
    noisy_rabi = (
        rabi(x,p0,p1,p2,p3,p4)+p1*np.random.randn(samples)* 1e-3
    )
    ...
    fit = rabi_fit(
        ...
        0,
        nqubits,
        labels=[label, "pulse_max_voltage"]
    )
    ...
    y_fit = (
        fit_p1 * np.sin(2 * np.pi * x * fit_p2 + fit_p3) 
    )
...
def test_ramsey_fit( amplitude_sign,caplog):
    ...
    x = np.linspace(0, 1/p2, samples)
    noisy_ramsey = (
        ramsey(x,p0,p1,p2,p3,p4)+p1*np.random.randn(samples)* 1e-3
    )
...
def test_flipping_fit( label, nqubits, amplitude_sign,caplog):
    ...
    x = np.linspace(0,10, 100)
    noisy_flip = (
        flipping(x, p0,p1,p2,p3)
        +  p0*np.random.randn(100) * 1e-4
    )
