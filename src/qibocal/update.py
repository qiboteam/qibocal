...
_QPAIR_DUMPER = TypeAdapter(QubitPairId)


def _dump_pair(pair: QubitPairId) -> str:
...
def drive_duration(
    ...
    if rx90:
        platform.update(
            {f"native_gates.single_qubit.{qubit}.RX90.0.1.duration": duration}
        )
    else:
        platform.update(
            {f"native_gates.single_qubit.{qubit}.RX.0.1.duration": duration}
        )
