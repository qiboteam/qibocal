...
# For calibrate qubit states
def qubit_states_trains(folder, routine, qubit, format):
    figures = []
    ...
    for subfolder in subfolders:
        ...
        try:
            ...
            average_state = complex(parameters.get_values("average_state")[0])
            rotation_angle = parameters.get_values("rotation_angle")[0]
            threshold = parameters.get_values("threshold")[0]
            fidelity = parameters.get_values("fidelity")[0]
            assignment_fidelity = parameters.get_values("assignment_fidelity")[0]
        ...
        ns = data.df["n"].unique()
        for n in ns:
            state_data = data.df[data.df["n"] == n]
            fig.add_trace(
                go.Scatter(
                    x=state_data["i"].pint.to("V").pint.magnitude,
                    y=state_data["q"].pint.to("V").pint.magnitude,
                    name=f"q{qubit}/r{report_n}: state {n}",
                    legendgroup=f"q{qubit}/r{report_n}: state {n}",
                    mode="markers",
                    showlegend=False,
                    opacity=0.7,
                    marker=dict(size=3, color=get_color_state0(report_n)),
            ...
            max_x = max(
                max_x,
                state_data["i"].pint.to("V").pint.magnitude.max(),
            )
            max_y = max(
                max_y,
                state_data["q"].pint.to("V").pint.magnitude.max(),
            )
            min_x = min(
                min_x,
                state_data["i"].pint.to("V").pint.magnitude.min(),
            )
            min_y = min(
                min_y,
                state_data["q"].pint.to("V").pint.magnitude.min(),
            )

            # fig.add_trace(
            #     go.Scatter(
            #         x=[average_state.real],
            #         y=[average_state.imag],
            #         name=f"q{qubit}/r{report_n}: state 0",
            #         legendgroup=f"q{qubit}/r{report_n}: state 0",
            #         showlegend=True,
            #         mode="markers",
