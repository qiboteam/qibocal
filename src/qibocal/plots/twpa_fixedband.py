...
def twpa_frequency(folder, routine, qubit, format):
    ...
    fig_fidelity.add_trace(
        go.Scatter(x=data_fit.df["delta_frequency"], y=data_fit.df["fidelity"])
    )
...
def twpa_power(folder, routine, qubit, format):
    ...
    for subfolder in subfolders:
        ...
        # Plot raw results with sliders
        annotations_dict = []
        for delta_power in data.df["delta_power"].unique():
            state0_data = data.df[
                (data.df["delta_power"] == delta_power) & (data.df["state"] == 0)
            ]
            state1_data = data.df[
                (data.df["delta_power"] == delta_power) & (data.df["state"] == 1)
            ]
            fit_data = data_fit.df[data_fit.df["delta_power"] == delta_power.magnitude]
            fit_data["average_state0"] = data_fit.df["average_state0"].apply(
                lambda x: complex(x)
            )
            fit_data["average_state1"] = data_fit.df["average_state1"].apply(
            ...
            fig.add_trace(
                go.Scatter(
                    x=[fit_data["average_state0"].apply(lambda x: np.real(x))],
                    y=[fit_data["average_state0"].apply(lambda x: np.imag(x))],
                    name=f"q{qubit}/r{report_n}: mean state 0",
                    legendgroup=f"q{qubit}/r{report_n}",
                    showlegend=True,
                    visible=False,
            ...
            fig.add_trace(
                go.Scatter(
                    x=[fit_data["average_state1"].apply(lambda x: np.real(x))],
                    y=[fit_data["average_state1"].apply(lambda x: np.imag(x))],
                    name=f"avg q{qubit}/r{report_n}: mean state 1",
                    legendgroup=f"q{qubit}/r{report_n}",
                    showlegend=True,
                    visible=False,
            ...
            # Add fitting report
            title_text = f"q{qubit}/r{report_n}/r{delta_power}<br>"
            # title_text += f"average state 0: ({fit_data['average_state0'][0]:.6f})<br>"
            # title_text += f"average state 1: ({fit_data['average_state1'][0]:.6f})<br>"
            # title_text += (
            #     f"rotation angle = {fit_data['rotation_angle'][0]:.3f} / threshold = {fit_data['threshold'][0]:.6f}<br>"
    ...
    fig.data[2].visible = True
    fig.data[3].visible = True
    # TODO: Show annotations for the first power

    # Add slider
    steps = []
    for i, freq in enumerate(data.df["delta_power"].unique()):
        step = dict(
            method="update",
            args=[
                {"visible": [False] * len(fig.data)},
            ...
            label=f"{data.df['delta_power'].unique()[i]:.6f}",
        )
        for j in range(4):
            step["args"][0]["visible"][i * 4 + j] = True
            step["args"][1]["annotations"][0][i * 4 + j] = True
    ...
    sliders = [
        dict(
            currentvalue={"prefix": "delta_power: "},
            steps=steps,
        )
    ]
    ...
    fig_fidelity.add_trace(
        go.Scatter(x=data_fit.df["delta_power"], y=data_fit.df["fidelity"])
    )
    fig_fidelity.update_layout(
        showlegend=True,
        uirevision="0",  # ``uirevision`` allows zooming while live plotting
