...
def grouped_by_mean(df, g_column1, g_column2, m_column1, m_column2):
    r"""
    ...
    unique_column1, column1_indexes = np.unique(data[:, g_column1], return_inverse=True)
    unique_column2, column2_indexes = np.unique(data[:, g_column2], return_inverse=True)
...
def grouped_by_mean_one(df, g_column: int, m_column: int):
    df = df.astype(float)
    data = df.to_numpy()

    # Group by column g_column and calculate mean of m_column
    unique_column, column_indexes = np.unique(data[:, g_column], return_inverse=True)
    data_m_column = data[:, m_column]
