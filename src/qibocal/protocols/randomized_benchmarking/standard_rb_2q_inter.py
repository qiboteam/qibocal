...
from qibocal.protocols.randomized_benchmarking.standard_rb import StandardRBParameters
...
from .utils import (
    RBType,
    add_inverse_2q_layer,
    add_measurement_layer,
    data_uncertainties,
    layer_2q_interleaved_circuit,
    number_to_str,
)
...
@dataclass
class StandardRB2QInterResult(StandardRB2QResult):
    """Standard RB outputs."""
...
def random_circuits(
    ...
    for _ in range(niter):
        for target in targets:
            circuit, random_index = layer_2q_interleaved_circuit(
                rb_gen, depth, target, interleaved
            )
...
def _fit(data: RB2QInterData) -> StandardRB2QInterResult:
    ...
    fidelity, fidelity_cz, pulse_fidelity = {}, {}, {}
    popts, perrs = {}, {}
    error_barss = {}
    for qubit in qubits:
        ...
        fid_cz = fidelity[qubit] / data.fidelity[qubit][0]
        uncertainty_cz = data.fidelity[qubit][1]
...
def _plot(
    data: RB2QData, fit: StandardRB2QInterResult, target: QubitPairId
