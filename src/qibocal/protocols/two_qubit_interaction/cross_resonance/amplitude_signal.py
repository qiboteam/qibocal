...
from qibolab.pulses import Pulse, Rectangular, PulseType, GaussianSquare, Gaussian
    ...
    for pair in targets:
        ...
        for ctr_setup in STATES:   
            for tgt_setup in STATES: 
                ...
                for qubit in pair:
                    ro_pulses[qubit] = platform.create_qubit_readout_pulse(qubit=qubit, start=cr_pulse.finish)
...
def _plot(data: CrossResonanceData, target: QubitPairId, fit: CrossResonanceResults):
    ...
    for ro_qubit in [tgt, ctr]:
        ...
        for ctr_setup in STATES:
            ...
            fig.add_trace(
                go.Scatter(
                    x=cr_parameters, y=i_data.signal, 
            ...
            fig.add_trace(
                go.Scatter(
                    x=cr_parameters, y=x_data.signal, 
...
cross_resonance_amplitude_signal = Routine(_acquisition, _fit, _plot)
