...
def calc_filter_taps(
    ...
    return _check_hardware_limitation(
        qop_version, feedforward_taps, list(feedback_taps)
    )
...
def highpass_correction(
    tau: float, Ts: float = 1, qop_version: QOPVersion = QOPVersion.get_latest()
):
    """
...
def _iir_correction(values, filter_type, feedforward_taps, feedback_taps, Ts=1.0):
    ...
    if filter_type == "highpass":
        for i, tau in enumerate(values):
            b[:, i], [feedback_taps[i]] = highpass_correction(
                tau, Ts, qop_version=QOPVersion.NONE
            )
    elif filter_type == "exponential":
        for i, (A, tau) in enumerate(values):
            b[:, i], [feedback_taps[i]] = single_exponential_correction(
                A, tau, Ts, qop_version=QOPVersion.NONE
            )
    else:
...
def _check_hardware_limitation(
    qop_version: QOPVersion, feedforward_taps: List, feedback_taps: List
):
    ...
    if len(feedforward_taps) > max_feedforward_len:
        ...
        if (
            np.max(np.sum(extra_taps)) / np.max(np.sum(final_taps)) > 0.10
        ):  # Contribution is more than 2%
            raise RuntimeError(
            ...
            np.max(np.sum(extra_taps)) / np.max(np.sum(final_taps)) > 0.02
        ):  # Contribution is more than 2%
            warnings.warn(
                f"The number of feedforward taps exceed the maximum length of "
                f"{qop_version.value['feedforward_length'](len(feedback_taps))}.\nRemoved all taps afterwards."
    ...
    if np.any(np.abs(feedback_taps) > qop_version.value["feedback_max"]):
        feedback_taps[feedback_taps > qop_version.value["feedback_max"]] = (
            qop_version.value["feedback_max"]
        )
        feedback_taps[feedback_taps < -qop_version.value["feedback_max"]] = (
            -qop_version.value["feedback_max"]
        )
    ...
    if max_value > qop_version.value["feedforward_max"]:
        feedforward_taps = (
            qop_version.value["feedforward_max"] * feedforward_taps / max_value
        )
        warn(
