...
def _plot(data: T1FluxData, target: QubitId, fit: T1FluxResults | None = None):
    """Plotting function for T1 flux experiment."""
    fig = go.Figure()
    if fit is not None:
        indices = list(set(np.where(~np.isnan(np.array(fit.t1[target])))[0]))
