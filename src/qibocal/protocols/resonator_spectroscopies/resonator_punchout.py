...
from ..utils import HZ_TO_GHZ, extract_feature, normalize_over_y, table_dict, table_html
...
class ResonatorPunchoutData(Data):
    ...
    def normalized_signal(self, qubit: QubitId) -> np.ndarray:
        raw_signal = self.signal(qubit)
        mins = raw_signal.min(axis=1, keepdims=True)
        maxs = raw_signal.max(axis=1, keepdims=True)
        return (raw_signal - mins) / (maxs - mins)
