...
def significant_digit(number: Number):
    ...
    Returns:
        int: position of the first significant digit or ``2`` if the given number
...
def number_to_str(
    ...
    if isinstance(uncertainty, Number):
        if precision is None:
            precision = significant_digit(uncertainty) + 1
    ...
    if precision is None:
        precision = max(significant_digit(error) + 1 for error in uncertainty)
...
def extract_from_data(
    ...
    return grouped_df.index.to_numpy(), np.array(grouped_df[output_key].values.tolist())
