...
class ResonatorFluxResults(Results):
    """ResonatorFlux outputs when fitting will be done."""
...
def _aquisition(
    ...
    sweeper_offset = Sweeper(
        Parameter.bias,
        delta_offset_range,
        qubits=[platform.qubits[f"c{qubit}"] for qubit in qubits],  # f"c{qubit}"
    )

    # create a DataUnits object to store the results,
    sweep_data = ResonatorFluxData()
...
def _fit(data: ResonatorFluxData):
    return ResonatorFluxResults()


resonator_flux = Routine(_aquisition, _fit, _plot)
