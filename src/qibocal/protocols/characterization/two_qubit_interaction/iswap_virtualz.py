"""iSWAP virtual correction experiment for two qubit gates, tune landscape."""
...
class iSWAPVirtualZParameters(Parameters):
    ...
    """Amplitude of flux pulse implementing iSWAP."""
...
@dataclass
class iSWAPVirtualZResults(Results):
    """iSWAPVirtualZ outputs when fitting will be done."""

    fitted_parameters: dict[tuple[str, QubitId],]
    """Fitted parameters"""
    iSWAP_angle: dict[QubitPairId, float]
    """iSWAP angle."""
    virtual_phase: dict[QubitPairId, dict[QubitId, float]]
    """Virtual Z phase correction."""


iSWAPVirtualZType = np.dtype([("target", np.float64), ("control", np.float64)])


@dataclass
class iSWAPVirtualZData(Data):
    """iSWAPVirtualZ data."""

    data: dict[tuple, npt.NDArray[iSWAPVirtualZType]] = field(default_factory=dict)
    ...
    def register_qubit(self, target, control, setup, prob_target, prob_control):
        ar = np.empty(prob_target.shape, dtype=iSWAPVirtualZType)
...
def create_sequence(
    ...
    iSWAP, virtual_z_phase = platform.create_iSWAP_pulse_sequence(
        (ordered_pair[1], ordered_pair[0]),
        start=max(Y90_pulse.finish, RX_pulse_start.finish),
    )

    if amplitude is not None:
        iSWAP.get_qubit_pulses(ordered_pair[1])[0].amplitude = amplitude

    theta_pulse = platform.create_RX90_pulse(
        target_qubit,
        start=iSWAP.finish + dt,
    ...
    sequence.add(
        Y90_pulse,
        iSWAP.get_qubit_pulses(ordered_pair[1]),
        iSWAP.coupler_pulses(platform.pairs[tuple(sorted(ordered_pair))].coupler.name),
        theta_pulse,
        measure_target,
        measure_control,
    )
    ...
    return (
        ...
        virtual_z_phase,
        theta_pulse,
        iSWAP.get_qubit_pulses(ordered_pair[1])[0].amplitude,
    )


def _acquisition(
    params: iSWAPVirtualZParameters,
    platform: Platform,
    qubits: Qubits,
) -> iSWAPVirtualZData:
    r"""
    Acquisition for iSWAPVirtualZ.
    ...
    data = iSWAPVirtualZData(thetas=theta_absolute.tolist())
    for pair in qubits:
...
def _fit(
    data: iSWAPVirtualZData,
) -> iSWAPVirtualZResults:
    ...
    for pair in pairs:
        ...
        for target_q, control_q in (
            ...
            iSWAP_angle[target_q, control_q] = abs(
    ...
    return iSWAPVirtualZResults(
        iSWAP_angle=iSWAP_angle,
...
# TODO: remove str
def _plot(data: iSWAPVirtualZData, fit: iSWAPVirtualZResults, qubit):
    """Plot routine for iSWAPVirtualZ."""
        ...
        if fit is not None:
            ...
            fitting_report = table_html(
                table_dict(
                    ...
                    ["iSWAP angle", "Virtual Z phase", "Flux pulse amplitude"],
                    [
                        np.round(fit.iSWAP_angle[target, control], 4),
...
def _update(results: iSWAPVirtualZResults, platform: Platform, qubit_pair: QubitPairId):
...
iswap_virtualz = Routine(_acquisition, _fit, _plot, _update, two_qubit_gates=True)
"""iSWAP virtual Z correction routine."""
