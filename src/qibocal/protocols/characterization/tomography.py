...
# def maxime_probabilities(result1_complex, result2_complex):
#     np.abs(
    ...
    for label1 in tomography_basis:
        for label2 in tomography_basis:
            if not (label1 == "RX" and label2 == "RX"):
                ...
                results1 = platform.execute_pulse_sequence(
                    total_sequence, ExecutionParameters(nshots=params.nshots)
                )
                ...
                results2 = platform.execute_pulse_sequence(
                    total_sequence, ExecutionParameters(nshots=params.nshots)
                )
