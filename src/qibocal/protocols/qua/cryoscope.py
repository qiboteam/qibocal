...
class CryoscopeQuaData(Data):
    ...
    @property
    def total_len(self):
        return self.const_flux_len + self.zeros_before_pulse + self.zeros_after_pulse


def baked_waveform(waveform, pulse_duration, config, flux):
...
def _acquisition(
    ...
    zeros_before_pulse = params.zeros_before_pulse
    zeros_after_pulse = params.zeros_after_pulse
    ...
    cos_iq = np.cos(iq_angle)
    sin_iq = np.sin(iq_angle)
        ...
        with for_(n, 0, n < params.nshots, n + 1):
            ...
            with for_(segment, 0, segment <= total_len, segment + 1):
                with for_each_(flag, [True, False]):
                    ...
                    assign(state, I * cos_iq - Q * sin_iq > threshold)
                    save(state, state_st)
            save(n, n_st)
...
cryoscope_qua = Routine(_acquisition, _fit, _plot, _update)
