...
native_2_qubit_gates = {
    "sqr_iSWAP": {
        ...
        "SWAP": [
            ...
            cirq.PhasedXZGate(axis_phase_exponent=0.5, x_exponent=0.5, z_exponent=0)(
                q2
            ),
            cirq.ISWAP(q1, q2) ** 0.5,
    ...
    "CNOT": {
        ...
        "iSWAP": [
            ...
            cirq.CNOT(q1, q2),
            cirq.CNOT(q2, q1),
            cirq.PhasedXZGate(axis_phase_exponent=0.0, x_exponent=0.0, z_exponent=0.0)(
                q1
            ),  # I
            cirq.PhasedXZGate(
...
def combine_to_phased_x_z(
    first_gate: cirq.GateOperation, second_gate: cirq.GateOperation
) -> cirq.GateOperation:
...
class _GateDatabase:
    def __init__(self):
        self._commands, self._tableaus, self._symplectic_range, self._pauli_range = (
            self._gen_commands_and_tableaus()
        )
    ...
    def _gen_commands_and_tableaus():
        ...
        for command in commands:
            if command[0] == "C1's":
                ...
                rb_commands.append(
                    GateCommand(
                        "CNOT", (command[1], command[3]), (command[2], command[4])
                    )
                )
            elif command[0] == "iSWAP's":
                rb_commands.append(
                    GateCommand(
                        "iSWAP", (command[1], command[3]), (command[2], command[4])
                    )
                )
        ...
        for i1 in range(len(pauli)):
            for i2 in range(len(pauli)):
                tableaus.append(
                    SimpleTableau(np.eye(4), pauli_phase[i1] + pauli_phase[i2])
                )
    ...
    def find_pauli_gate_id_by_tableau_alpha(self, tableau: SimpleTableau):
        ...
        return self._pauli_range[0] + next(
            i for i, x in enumerate(tableaus) if np.array_equal(x.alpha, tableau.alpha)
        )
...
class GateGenerator:
    ...
    def _generate_two_qubit_dict(native_two_qubit_gates: Set[str]) -> dict:
        ...
        for k, v in GateGenerator.two_qubit_imp_priority.items():
            ...
            if (
                len(available_imp) == 0
                or available_imp[0] not in native_2_qubit_gates.keys()
            ):
                raise RuntimeError(
                    f"Cannot implement gate '{k}' with provided native two qubit gates"
                )
    ...
    def _reduce_gate(gate: List[cirq.GateOperation]):
        ...
        for op in gate:
            if len(op.qubits) == 1:
                ...
                qubit_ops[op.qubits[0].x] = (
                    op if prev_op is None else combine_to_phased_x_z(prev_op, op)
                )
            else:
                append_qubit_ops()
                output.append(op)
    ...
    def generate(self, cmd_id):
        ...
        two_qubit_imp = (
            self._two_qubit_dict[command.type]
            if command.type in self._two_qubit_dict
            else None
        )
        if command.type == "C1":
