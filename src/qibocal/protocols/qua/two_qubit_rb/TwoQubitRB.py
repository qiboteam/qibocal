...
class TwoQubitRb:
    ...
    def __init__(
        ...
        two_qubit_gate_generators: Dict[
            Literal["sqr_iSWAP", "CNOT", "CZ"], Callable[[Baking, int, int], None]
        ],
        ...
        self._interleaving_tableau = (
            tableau_from_cirq(interleaving_gate)
    ...
    def _gen_rb_sequence(self, depth):
        ...
        for i in range(depth):
            ...
            tableau = tableau.then(gate_db.get_tableau(symplectic)).then(
                gate_db.get_tableau(pauli)
            )
    ...
    def _gen_qua_program(
        ...
        with program() as prog:
            ...
            with stream_processing():
                state_os.buffer(len(sequence_depths), num_repeats, num_averages).save(
                    "state"
                )
    ...
    def run(
        ...
        **kwargs,
        ...
        gen_sequence_callback = (
            kwargs["gen_sequence_callback"]
            if "gen_sequence_callback" in kwargs
            else None
        )
        self._insert_all_input_stream(
            job, circuit_depths, num_circuits_per_depth, gen_sequence_callback
        )
