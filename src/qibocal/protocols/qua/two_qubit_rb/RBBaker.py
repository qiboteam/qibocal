...
from qm.qua import align, case_, declare, for_, switch_
...
class RBBaker:
    ...
    def _gen_gate(self, baker: Baking, gate_op: GateOperation):
        if type(gate_op.gate) == cirq.PhasedXZGate:
            self._single_qubit_gate_generator(
                baker, self._get_qubits(gate_op)[0], *self._get_phased_xz_args(gate_op)
            )
        elif type(gate_op.gate) == cirq.ISwapPowGate and gate_op.gate.exponent == 0.5:
            self._validate_two_qubit_gate_available("sqr_iSWAP")
            self._two_qubit_gate_generators["sqr_iSWAP"](
                baker, *self._get_qubits(gate_op)
            )
    ...
    def gates_from_cmd_id(self, cmd_id):
        ...
        elif self._interleaving_gate is not None and cmd_id == len(
            gate_db.commands
        ):  # Interleaving gate
    ...
    def _bake_all_ops(self, config: dict):
        ...
        num_of_commands = len(gate_db.commands) + (
            0 if self._interleaving_gate is None else 1
        )
        for cmd_id in tqdm(
            range(num_of_commands),
            desc="Baking pulses which comprise Cliffords",
            unit="command",
        ):
            with baking(config) as b:
    ...
    def run(self, op_list_per_qe: dict, length, unsafe=True):
        if set(op_list_per_qe.keys()) != self._all_elements:
            raise RuntimeError(
                f"must specify ops for all elements: {', '.join(self._all_elements)} "
            )
