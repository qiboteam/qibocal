...
_clifford_compose_2q_data = np.load(
    f"{_DATA_FOLDER}/clifford_compose_2q_dense_selected.npz"
)
...
def _transpile_clifford_circuit(
    ...
    return _apply_qubit_layout(
        _decompose_clifford_ops(circuit), physical_qubits=physical_qubits
    )
...
def _apply_qubit_layout(
    circuit: QuantumCircuit, physical_qubits: Sequence[int]
) -> QuantumCircuit:
    # Mapping qubits in circuit to physical qubits (layout)
    res = QuantumCircuit(
        1 + max(physical_qubits), name=circuit.name, metadata=circuit.metadata
    )
...
def _synthesize_clifford_circuit(
    ...
    pm = PassManager(
        [HighLevelSynthesis(hls_config=hls_config, coupling_map=coupling_map)]
    )
...
class CliffordUtils:
    ...
    def clifford_1_qubit_circuit(
        ...
        if basis_gates:
            qc = _synthesize_clifford_circuit(
