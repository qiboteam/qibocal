...
def exp2_func(
        x: np.ndarray, A1: float, A2: float, f1: float, f2: float) -> np.ndarray:
    """ """
...
def esprit(xdata, ydata, num_decays, hankel_dim=None):
        ...
        if hankel_dim is None:
            hankel_dim = int(np.round(.5 * xdata.size)) 
...
def fit_exp1_func(
    ...
    try:
        popt, _ = curve_fit(
            exp1_func, xdata, ydata, p0=guess, method="lm"
        )
    except:
        popt = (0, 0, 0)
...
def fit_exp2_func(
    ...
    if sum(decays<0):
        ...
        dtype = float
    x_fit = np.linspace(
        np.sort(xdata)[0], np.sort(xdata)[-1],
         num=len(xdata) * 20, dtype = dtype)
    # Get the ydata for the fit with the calculated parameters.
