...
def fast_reset_MSR(
    ...
    data = DataUnits(name="data", options=["qubit", "iteration", "n", "fast_reset"])
        ...
        r.update(
            {
                ...
                "n": [1] * nshots,
                "fast_reset": ["True"] * nshots,
            }
        )
        data.add_data_from_dict(r)

    results_nfr = platform.execute_pulse_sequence(
        sequence,
        nshots=nshots,
        relaxation_time=relaxation_time,
    )
    ...
    # retrieve and store the results for every qubit
    for ro_pulse in ro_pulses.values():
        r = results_nfr[ro_pulse.serial].raw
        r.update(
            {
                "qubit": [ro_pulse.qubit] * nshots,
                "iteration": np.arange(nshots),
                "n": [1] * nshots,
                "fast_reset": ["False"] * nshots,
            }
        )
        data.add_data_from_dict(r)
