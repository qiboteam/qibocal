...
def qubit_spectroscopy(
    ...
    for qubit in qubits:
        ...
        qd_pulses[qubit].amplitude *= drive_amplitude_factor
...
@plot(
    "MSR and Phase vs Qubit Drive Frequency 2-photon Transition 0->2",
    plots.frequency_msr_phase,
)
def qubit_spectroscopy_02(
    platform: AbstractPlatform,
    qubits: dict,
    freq_width: int,
    freq_step: int,
...
def qubit_spectroscopy_flux(
    ...
    delta_frequency_range = np.arange(-freq_width * 3 // 4, freq_width // 4, freq_step)
        ...
        if diagonal_only:
            ...
            for fluxline in fluxlines:
                ...
                for qubit in qubits:
                    ...
                    freqs = np.tile(
                        delta_frequency_range + qd_pulses[qubit].frequency,
                        len(delta_bias_range),
                    )
                    # freqs = np.array(
                    #     len(delta_bias_range)
                    #     * list(delta_frequency_range + qd_pulses[qubit].frequency)
                    # ).flatten()
                    # biases = np.repeat(delta_bias_range + platform.get_bias(fluxline), len(delta_frequency_range))
                    # ndata = len(delta_bias_range) * len(delta_frequency_range)
                    r = {k: v.ravel() for k, v in result.raw.items()}
                    r.update(
                        {
                            "frequency[Hz]": freqs,
...
@plot(
    "MSR and Phase vs Qubit Drive Frequency and Flux Current - 2-photon Transition 0->2",
    plots.frequency_flux_msr_phase,
)
def qubit_spectroscopy_flux_02(
    platform: AbstractPlatform,
    qubits: dict,
    freq_width,
    freq_step,
