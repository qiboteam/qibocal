...
The calibration procedure is based on identifying the full effective Hamiltonian generated by the cross-resonance (CR) drive rather than optimizing the gate through trial and error. By measuring the target-qubit response for different states of the control qubit, the strengths of both the desired interaction and the accompanying unwanted terms are extracted. These parasitic terms arise from drive crosstalk and off-resonant interactions and can significantly affect gate performance. A cancellation (rotary) drive is then applied to the target qubit, with its amplitude and phase tuned to suppress the unwanted Hamiltonian components while preserving the desired ZX interaction. The overall strategy is therefore to characterize the error channels at the Hamiltonian level and compensate them through calibrated control pulses.


The idea behind this paper is to minimize all hamiltonian terms described above but ZX-term
This procedure relies on a tomography-like experiment on the target qubit it is possible to reconstruct the effective Hamiltonian of the system:
