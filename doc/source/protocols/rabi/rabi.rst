...
Since many routines and protocols in quantum computing are based on :math:`R_x(\pi/2)` rotations, in qibocal we implemented
...
`relaxation_time` higher than the qubit :math:`T1`.
...
to excite the qubit from the ground state up to state :math:`\frac{\ket{0}-i\ket{1}}{\sqrt{2}}` by simply setting the :math:`rx90` parameter to `True`.
